...
video_root = params["video_folder"]
out_root = params["out_folder"]
...
videos = [
    vn for vn in glob.glob(os.path.join(video_root, "**", f"*.{ftype}"), recursive=True) ]
...
for i,v in enumerate(videos):
    ...
    outpath_vid = os.path.join(out_video, os.path.relpath(v, video_root))
    os.makedirs(os.path.dirname(outpath_vid), exist_ok=True)
...
if process_labeled:
    ...
    labeled_folders = [
        folder for folder in glob.glob(os.path.join(params["labeled_data_folder"], "*")) 
